...
async def select_avatar(request: SelectAvatarRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if adapter_exists:
            ...
            success = model_manager.attach_existing_adapter(avatar)
        ...
        if not request.use_base_model and model_manager.current_avatar != "base":
            if model_manager.current_adapter != model_manager.current_avatar:
                ...
                if not os.path.exists(os.path.join(adapter_path, "adapter_config.json")):
                    ...
                    model_manager.attach_existing_adapter(model_manager.current_avatar)
...
@app.post("/add_documents_bulk", tags=["ChromaDB Document Management"])
async def add_documents_bulk(request: BulkDocumentRequest):
    ...
    try:
        ...
        for doc_request in request.documents:
            ...
            documents.append(doc_request.content)
            ...
            if doc_request.metadata:
                metadata.update(doc_request.metadata)
        ...
        await websocket_manager.broadcast({
            "type": "documents_bulk_added",
            "avatar": model_manager.current_avatar,
            "count": len(doc_ids)
        })
        
        return {
            "status": f"{len(doc_ids)} documents added successfully",
            "document_ids": doc_ids,
            "avatar": model_manager.current_avatar
        }
        
    except Exception as e:
        logger.error(f"Error adding documents in bulk: {str(e)}")
